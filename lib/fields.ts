...
export const ASSIGNED_LAPTOPS_ID = "assigned-laptops";

/** Sheets the user is not allowed to delete (built-ins + virtual views). */
...
export function isAssignedLaptop(r: RecordRow): boolean {
  return (
    String(r.category ?? "") === "Laptop" &&
    String(r.assignedTo ?? "").trim() !== ""
  );
}

export function buildAssignedLaptopsSheet(cnt: number): Sheet {
  return {
    id: ASSIGNED_LAPTOPS_ID,
    name: "Assigned Laptops",
    color: "#7A4FE0",
    icon: "laptop",
    type: "hardware",
    fields: [],
    sort: 1.5, // sits right after Hardware
    cnt,
  };
}
